...
public class DeviceSensor implements SensorEventListener {
    ...
    private final Sensor compassEvent;
    ...
    private DeviceSensor(Context context) {
        ...
        compassEvent = sensorManager.getDefaultSensor(Sensor.TYPE_ORIENTATION);
    ...
    public void enableSensor() {
        ...
        if (!isEnabled) {
            ...
            sensorManager.registerListener(this,compassEvent,SensorManager.SENSOR_DELAY_GAME);
            sensorManager.registerListener(this, magnetometer, SensorManager.SENSOR_DELAY_NORMAL);
            tripId = System.currentTimeMillis()/1000;
            app.setTripId(tripId);
            timer = new Timer();
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        if (sensorEvent.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            Log.v(TAG, "Compass -- degree: " + sensorEvent.values[0]);
    ...
    private class SendTimerTask extends TimerTask {
        ...
        public void run() {
            ...
            if (app.getCurrentLocation() != null) {
                ...
                acc = app.getCurrentLocation().getAccuracy();
            }
            app.setlocationaccuracy(acc);
            String messageData = MessageFactory.getAccelMessage(G, O, yaw, lon, lat, heading, speed, tripId,C,acc);
            ...
            try {
                ...
                if (app.getConnectionType() == Constants.ConnectionType.QUICKSTART) {
                    iotClient.publishEvent(Constants.STATUS_EVENT, "json", messageData, 2, false, listener);
                } else {
                    iotClient.publishEvent(Constants.GPS_EVENT, "json", messageData, 2, false, listener);
                }
            ...
            //if (runningActivity != null && runningActivity.equals(IoTPagerFragment.class.getName())) {
            Intent actionIntent = new Intent(Constants.APP_ID + Constants.INTENT_IOT);
            actionIntent.putExtra(Constants.INTENT_DATA, Constants.GPS_EVENT);
            context.sendBroadcast(actionIntent);
            //}
        }
    }
